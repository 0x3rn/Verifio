...
import { clearFreshAuthNavigation } from '@/lib/fresh-auth-navigation';
...
export default function DashboardClient({ skipInitialSkeleton = false }: { skipInitialSkeleton?: boolean }) {
  ...
  useEffect(() => {
    if (skipInitialSkeleton) clearFreshAuthNavigation();
  }, [skipInitialSkeleton]);
  ...
  if (loading && !skipInitialSkeleton) {
    ...
    return (
      <div className="dash-layout">
        <div className="dash-load-state" role="status">
          ...
          <button type="button" className="dash-btn-primary" onClick={() => setAuthReloadToken((token) => token + 1)}>Try again now</button>
        </div>
      </div>
    );
  }
    ...
    <div className="dash-layout">
      ...
      <header className="dash-header">
        ...
        <div className="dash-header__balance">
          ...
          <span>{user ? `$${user.balance.toFixed(2)}` : 'Checking account…'}</span>
